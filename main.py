...
import joblib
from stuff_plus_model import score_stuff_plus, diagnose_stuff_plus, build_metric_percentile_summary

app = FastAPI()
openai_api_key = os.environ.get("OPENAI_API_KEY")
client = OpenAI(api_key=openai_api_key) if openai_api_key else None
stuff_plus_artifacts = None
stuff_plus_model_path = os.environ.get("STUFF_PLUS_MODEL_PATH", "models/stuff_plus_mlb.joblib")
if os.path.exists(stuff_plus_model_path):
    stuff_plus_artifacts = joblib.load(stuff_plus_model_path)
...
async def metadata(file: UploadFile = File(...)):
    ...
    def uniq(col):
        ...
        values = values[values != ""]  # remove empty
        return sorted(values.unique().tolist())

    # --- Normalize pitch types (fix ChangeUp vs Changeup) ---
    def uniq_pitch(col):
        if col not in df.columns:
            return []
        ...
        # convert back to nice format (Changeup instead of changeup)
        values = values.str.capitalize()
    ...
    if "Date" in df.columns:
        ...
        if d.notna().any():
            ...
            date_options = sorted(d.dropna().dt.date.astype(str).unique().tolist())

    return {
        "pitchers": uniq("Pitcher"),
        "batters": uniq("Batter"),
        "tagged_pitch_types": uniq_pitch("TaggedPitchType"),
        "auto_pitch_types": uniq_pitch("AutoPitchType"),
        "hitter_handedness": uniq("BatterSide"),
        "counts": [
            "0-0", "0-1", "0-2",
            "1-0", "1-1", "1-2",
            "2-0", "2-1", "2-2",
...
async def heatmap_upload(
    ...
    count: str = Form(None),              # example: "1-2"
    batter_side: str = Form(None),        # "L" or "R"
    date_from: str = Form(None),          # "YYYY-MM-DD"
    date_to: str = Form(None),            # "YYYY-MM-DD"
):
    df = pd.read_csv(file.file, low_memory=False)

    if pitcher and "Pitcher" in df.columns:
    ...
    pitch_col = "AutoPitchType" if pitch_type_source == "auto" else "TaggedPitchType"
...
@app.post("/stuffplus/score")
async def stuffplus_score(
    file: UploadFile = File(...),
    pitcher: str = Form(None),
    pitch_type: str = Form(None),
    pitch_type_source: str = Form("tagged"),
    min_pitches: int = Form(10),
):
    if stuff_plus_artifacts is None:
        raise HTTPException(
            status_code=503,
            detail="Stuff+ model not loaded. Train one with train_stuff_plus_mlb.py and set STUFF_PLUS_MODEL_PATH.",
        )
    ...
    pitch_level, summary = score_stuff_plus(df, stuff_plus_artifacts)
    metric_percentiles = build_metric_percentile_summary(pitch_level, stuff_plus_artifacts)

    min_pitches = max(int(min_pitches), 1)
    if len(summary) > 0 and "pitches" in summary.columns:
        summary = summary[summary["pitches"] >= min_pitches].copy()
    metric_percentiles = [r for r in metric_percentiles if int(r.get("pitches", 0)) >= min_pitches]

    cols = [c for c in ["Pitcher", "pitch_type", "pitch_bucket", "stuff_plus", "pred_run_value"] if c in pitch_level.columns]
    pitch_level_records = pitch_level[cols].head(5000).replace({np.nan: None}).to_dict(orient="records")
    summary_records = summary.replace({np.nan: None}).to_dict(orient="records")

    return json_safe({
        "model_path": stuff_plus_model_path,
        "mlb_baseline": {
            "mean_pred_run_value": stuff_plus_artifacts.mlb_mean_pred,
            "std_pred_run_value": stuff_plus_artifacts.mlb_std_pred,
            "bucket_baselines": getattr(stuff_plus_artifacts, "bucket_baselines", {}),
        },
        "min_pitches": min_pitches,
        "rows_scored": int(len(pitch_level)),
        "summary": summary_records,
        "metric_percentiles": metric_percentiles,
        "pitch_level_sample": pitch_level_records,
    })


@app.post("/stuffplus/diagnostics")
async def stuffplus_diagnostics(
    file: UploadFile = File(...),
    pitcher: str = Form(None),
    pitch_type: str = Form(None),
    pitch_type_source: str = Form("tagged"),
    min_pitches: int = Form(20),
    top_k: int = Form(3),
):
    if stuff_plus_artifacts is None:
        raise HTTPException(
    ...
    diagnostics = diagnose_stuff_plus(
        df,
        stuff_plus_artifacts,
        min_pitches=max(int(min_pitches), 1),
    ...
    return json_safe({
        ...
        "top_k": int(top_k),
        "diagnostics": diagnostics.replace({np.nan: None}).to_dict(orient="records"),
    })